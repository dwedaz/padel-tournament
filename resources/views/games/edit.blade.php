<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Game: Set ' . $game->set . ' - Match ' . $game->game_set) }}
        </h2>
    </x-slot>
                        ...
                        <div>
                            ...
                            <select name="field_id" 
                                ...
                                @foreach($fields as $field)
                                    <option value="{{ $field->id }}" {{ old('field_id', $game->field_id) == $field->id ? 'selected' : '' }}>
                                        {{ $field->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div>
                                ...
                                <select name="winner_id" 
                                    ...
                                    @foreach($teamsByGroup as $groupName => $groupTeams)
                                        <optgroup label="{{ $groupName }}">
                                            @foreach($groupTeams as $team)
                                                <option value="{{ $team->id }}" {{ old('winner_id', $game->winner_id) == $team->id ? 'selected' : '' }}>
                                                    {{ $team->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
